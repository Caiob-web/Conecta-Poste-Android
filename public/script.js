...
function corPorEmpresas(q){ if(Number(q)>5) return "red"; if(Number(q)>=4) return "yellow"; return "green"; }
...
function addBatch(items, start=0, batch=1200){
  ...
  for(let i=start;i<end;i++){
    ...
    const m=L.circleMarker(ll,{radius:6, fillColor:corPorEmpresas(qtd), color:"#fff", weight:2, fillOpacity:0.9})
      .bindPopup(`
...
async function loadVisible(){
  ...
  try{
    ...
    if(!firstLoadDone){ firstLoadDone=true; hideOverlay(); initClockAndWeather(); }
  }catch(e){
...
/* Botão: minha localização (pino azul + precisão) */
let myLocMarker=null, myLocCircle=null;
document.getElementById("localizacaoUsuario")?.addEventListener("click",(e)=>{
  e.preventDefault(); e.stopPropagation();
  ...
  navigator.geolocation.getCurrentPosition((pos)=>{
    ...
    if(!myLocCircle){
      myLocCircle=L.circle(ll,{radius:accuracy||30,color:"#2196f3",fillColor:"#2196f3",fillOpacity:.15,weight:1}).addTo(map);
    } else {
    ...
    console.warn(err); alert("Não foi possível obter sua localização.");
...
if(!dataSmall && horaDiv){
  ...
  Object.assign(dataSmall.style,{marginLeft:"6px",fontSize:"12px",opacity:"0.8",fontWeight:"normal"});
...
function formatShortDate(d){ let s=d.toLocaleDateString("pt-BR",{weekday:"short",day:"2-digit",month:"2-digit"}); return s.replace(/\.$/,"").toLowerCase(); }
function startClock(){ const tick=()=>{ const n=new Date(); if(horaSpan) horaSpan.textContent=n.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}); if(dataSmall) dataSmall.textContent=` ${formatShortDate(n)}`; }; tick(); setInterval(tick,1000); }
function wxDesc(code){ const c=+code; if([0].includes(c)) return "Céu limpo"; if([1,2].includes(c)) return "Parcialmente nublado"; if([3].includes(c)) return "Nublado"; if([45,48].includes(c)) return "Neblina"; if([51,53,55].includes(c)) return "Garoa"; if([61,63,65].includes(c)) return "Chuva"; if([80,81,82].includes(c)) return "Pancadas de chuva"; if([71,73,75,77].includes(c)) return "Neve"; if([95,96,99].includes(c)) return "Tempestade"; return "Tempo indefinido"; }
function wxIconDataURI(code, isDay){ const c=+code; const sun=`<circle cx="16" cy="16" r="6" fill="${isDay?'#FDB813':'#B0C4DE'}"/>`; const cloud=`<ellipse cx="18" cy="18" rx="10" ry="6" fill="#cfd8dc"/>`; const drops=`<path d="M10 26 l2 -4 l2 4 z M18 26 l2 -4 l2 4 z M26 26 l2 -4 l2 4 z" fill="#4fc3f7"/>`; const bolt=`<polygon points="18,16 14,24 20,24 16,32 26,22 20,22 24,16" fill="#fdd835"/>`; let inner=''; if(c===0) inner=sun; else if([1,2].includes(c)) inner=`${sun}${cloud}`; else if([3,45,48].includes(c)) inner=cloud; else if([51,53,55,61,63,65,80,81,82].includes(c)) inner=`${cloud}${drops}`; else if([95,96,99].includes(c)) inner=`${cloud}${bolt}`; else inner=`${cloud}`; const svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36">${inner}</svg>`; return `data:image/svg+xml;utf8,${encodeURIComponent(svg)}`; }
...
async function reverseGeocodeName(lat,lon){
  ...
  try{
    ...
    geoCache.set(key, place); return place;
...
async function refreshWeather(lat,lon){
  try{
    ...
    const desc=wxDesc(c.weather_code), temp=Math.round(c.temperature_2m);
  ...
  }catch(e){ if(tempoSpan) tempoSpan.textContent="Clima indisponível"; if(tempoImg) tempoImg.style.display="none"; console.warn(e); }
}
function initClockAndWeather(){
  if(widgetStarted) return; widgetStarted=true;
  startClock();
  const c=map.getCenter(); refreshWeather(c.lat,c.lng);
  setInterval(()=>{ const cc=map.getCenter(); refreshWeather(cc.lat,cc.lng); }, 10*60*1000);
  document.querySelector("#widget-clima #tempo")?.addEventListener("click",()=>{ const cc=map.getCenter(); refreshWeather(cc.lat,cc.lng); });
}
